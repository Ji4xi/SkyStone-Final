...
public class NewSkyStoneIntakeRed extends StoneFinder {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        else if (skystoneAngle == 0) YLinearInchesGyro(-16.1 - 8.5 + 3 * TILE_INCH + 3 * SKYSTONE_INCH, GAGE.REVERSE, drivePwrMax + 0.3, 90); //DANGEROUS
        else {
        ...
        topClaw.setPosition(0.257);
        bottomClaw.setPosition(0);
        if (skystoneAngle > 0) XLinearInchesGyro(drift + 4.0, Side.LEFT, drivePwrMax - 0.3, -90);
        else if (skystoneAngle < 0) XLinearInchesGyro(drift + 4.3, Side.LEFT, drivePwrMax - 0.3, -90);
        else XLinearInchesGyro(drift + 3.3, Side.LEFT, drivePwrMax - 0.3, -90);

        topClaw.setPosition(0);
        sleep(400);
        //RAISE
        moveClaw(0, 0.33);
        if (skystoneAngle > 0) XLinearInchesGyro(drift + 4.8, Side.RIGHT, drivePwrMax - 0.3, -90);
        else if (skystoneAngle < 0) XLinearInchesGyro(drift + 4.6, Side.RIGHT, drivePwrMax - 0.3, -90);
        else
        XLinearInchesGyro(drift + 4.1, Side.RIGHT, drivePwrMax - 0.3, -90);

        //need fix in distance travelled
        if (skystoneAngle > 0)
            YLinearInchesGyro(0.9 - 11.5 + 3 * TILE_INCH + 2 * SKYSTONE_INCH, GAGE.FORWARD, drivePwrMax + 0.3, -90); //DANGEROUS
        else if (skystoneAngle == 0) YLinearInchesGyro(0.9 - 11.5 + 3 * TILE_INCH + 3 * SKYSTONE_INCH, GAGE.FORWARD, drivePwrMax + 0.3, -90); //DANGEROUS
        else {
        ...
        else if (skystoneAngle < 0) XLinearInchesGyro(drift + 2.95, Side.LEFT, drivePwrMax - 0.3, -90);
        ...
        //extend servo
        //tape.setPower(0.4);
        YLinearInchesGyro(TILE_INCH + 9.9, GAGE.FORWARD, drivePwrMax , 180);
